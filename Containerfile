...
# Verification image for the Julia package. KnotTheory.jl is pinned at source:
# it is the weakdep behind the KnotTheoryExt tests, not evidence of a shared
# database purpose between the projects.
...
COPY ext/ ext/
COPY test/ test/
RUN julia --startup-file=no --project=. -e 'using Pkg; Pkg.Registry.add("General"); Pkg.develop(path="/workspace/KnotTheory.jl"); Pkg.instantiate(); Pkg.test()'
