...
[NOTE]
====
*Role in the KRL stack.* Skein.jl is the *computational / backend engine* of the
https://github.com/hyperpolymath/krl[KRL] (Knot Resolution Language) stack: it
computes, transforms, normalizes, and evaluates invariants and equivalence checks,
with an embedded SQLite store. *QuandleDB* is the database application that wraps
Skein and is the canonical persistence + invariant/equivalence face. The four KRL
operations (construct / transform / resolve / retrieve) run against the
QuandleDB+Skein substrate — no single op maps to one component. Generic query access
here is an engine-layer affordance; KRL itself is a resolution DSL, *not merely* a
query language.
====
